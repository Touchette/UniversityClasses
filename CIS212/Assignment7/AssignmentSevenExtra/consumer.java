...
public class consumer implements Runnable {
	...
	public Object Mutex;
	// A reference to producer to check how many strings total have been produced by accessing the
	// variable "i" in the producer class
	producer producer = new producer(linkedQueue, running, Mutex);
	...
	public consumer(LinkedList<String> queue, boolean completed, Object monitor) {
		linkedQueue = queue;
		running = completed;
		Mutex = monitor;
	}
	...
	public synchronized void run() {
		...
		while(running && i<=1000) {
			...
			// This condition checks if the queue is empty and the producers still need to produce strings,
			// if so it notifies all producers working on the linkedQueue object
			synchronized (Mutex) {
				if (linkedQueue.size() == 0 && producer.i < 1000) {
					Mutex.notifyAll();
				}
			}
			
			// Locking the try/catch consume so that only one consumer may consume at a time
			this.lock.lock();
			try {
	...
	public void consume(int givenint) throws InterruptedException {
		...
		// This seems to cause a problem with an error in the amount of items consumed, the higher that the
		...
		// Takes the string from the queue using removeLast, first checking if there are items in the queue
		if (linkedQueue.size() > 0) {
			String taken = linkedQueue.pollLast();
